...
function sortAndListData(props) {
  ...
  for (var project in LastDateData) {
    if (sponsor === "all") {
      sortable.push({
        'name': project, 'followers': LastDateData[project].total_followers,
...
const ProjectModal = ({ projAccount }) => {

  const [isModalOpen, setIsModalOpen] = useState(false)

  // const fetchData = async (projectName) => {
  //   const res = await fetch(
  //     `https://fastapi-hrk.herokuapp.com/projects/history/${projectName}`,
  //   );
  ...
  console.log("inside project loop", data, projAccount)
...
const ProjectListing = () => {
  ...
  console.log("Proj List", projList)
      ...
      <Col lg={12}>
        <div className='wrapper'>
          {projList.map((project, i) => {
            console.log("inside project loop", project.name)
            return (
              <ProjectModal
                key={`project-${i}`}
